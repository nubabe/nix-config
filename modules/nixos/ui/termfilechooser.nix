...

{
  ...
  flake.modules.nixos.termfilechooser =
    ...
    let
      ...
      cfg = config.nubabe.ui.termfilechooser;
    in

    {
      options.nubabe.ui.termfilechooser = {
    ...
    }:
    {
      xdg.configFile."xdg-desktop-portal-termfilechooser/config".source =
        (pkgs.formats.ini { }).generate "config"
          {
            filechooser = {
              cmd = "${osConfig.nubabe.ui.termfilechooser.package}/share/xdg-desktop-portal-termfilechooser/yazi-wrapper.sh";
              default_dir = "$HOME";
              env = "TERMCMD=${osConfig.nubabe.ui.terminal.default} --title=termfilechooser -e";
              open_mode = "suggested";
              save_mode = "suggested";
            };
          };
